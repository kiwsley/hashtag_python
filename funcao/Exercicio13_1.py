...
def separar_listas(precos_imoveis, tamanho_imoveis, fator=0.1):
    if len(precos_imoveis) == len(tamanho_imoveis):
        i = int ((1-fator)*len(precos_imoveis))

        precos_imoveis_treinos = precos_imoveis[:i]
        precos_imoveis_testes = precos_imoveis[i:]

        tamanho_imoveis_treino = tamanho_imoveis[:i]
        tamanho_imoveis_teste = tamanho_imoveis[i:]

        return precos_imoveis_treinos,precos_imoveis_testes, tamanho_imoveis_treino,tamanho_imoveis_teste
    else: 
        print(" As listas não são do mesmo tamanho")   
